...
namespace TOHE.Roles.AddOns.Common
{
    public static class Evader
    {
        ...
        public static void SetupCustomOptions()
        {
            ...
            ChanceToEvadeVote = IntegerOptionItem.Create(Id + 13, "ChanceToEvadeVote", new(0, 100, 5), 50, TabGroup.Addons, false)
                .SetParent(CustomRoleSpawnChances[CustomRoles.Evader])
                .SetValueFormat(OptionFormat.Percent);
            ImpCanBeEvader = BooleanOptionItem.Create(Id + 10, "ImpCanBeEvader", true, TabGroup.Addons, false)
                .SetParent(CustomRoleSpawnChances[CustomRoles.Evader]);
            CrewCanBeEvader = BooleanOptionItem.Create(Id + 11, "CrewCanBeEvader", true, TabGroup.Addons, false)
                .SetParent(CustomRoleSpawnChances[CustomRoles.Evader]);
            NeutralCanBeEvader = BooleanOptionItem.Create(Id + 12, "NeutralCanBeEvader", true, TabGroup.Addons, false)
                .SetParent(CustomRoleSpawnChances[CustomRoles.Evader]);
        }
        ...
        {
            EvadeChance();
            if (Evade.TryGetValue(target.PlayerId, out bool didEvade) && didEvade)
            {
